...
class AMPInstagram implements TagConverterInterface
{
    ...
    public function convert(\DOMDocument $doc)
    {
        ...
        // /instagram.com\/(p|tv)\/([0-9a-zA-Z_\-]+)/i
        $query = '//blockquote[@class="instagram-media" and php:functionString(\'preg_match\', \'/instagram.com\/(p|tv)\/([0-9a-zA-Z_\-]+)/i\', @data-instgrm-permalink) > 0]';
        ...
        foreach ($entries as $tag) {
            preg_match('/instagram.com\/(p|tv)\/([0-9a-zA-Z_\-]+)/i', $tag->getAttribute('data-instgrm-permalink'), $id_match);
            $post_id = $id_match[2];
